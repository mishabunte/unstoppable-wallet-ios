...
enum UnlinkModule {
    static func viewController(account: Account) -> UIViewController {
        ...
        let viewController = account.watchAccount ? UnlinkWatchViewController(viewModel: viewModel) : (account.hardwareWallet ? UnlinkHardwareWalletViewController(viewModel: viewModel) : UnlinkViewController(viewModel: viewModel))
